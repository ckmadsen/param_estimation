...
# init_P = [0] # Initial condition
# degradation_timecourse_minutes = 0
# degradation_timecourse_protein = 0
...
def constant_expression(y, t, *args):
    ...
    k_loss = args[1] # A summation of dilution and degradation
    
    # Assign current values
    protein = y
    
    # Calculate change in values
    dprotein = k_express - protein*k_loss
    
    # Apply change in values to the current values in the next time step
    return dprotein

# Calculate k_express from k_loss and steady state data
def calculate_kexpress(SS, k_loss):
    return (SS * k_loss)

# Compute residuals in the degradation function
def residuals_degradation_timecourse(p, init_P, degradation_timecourse_minutes, degradation_timecourse_protein):
    p = tuple(p)
    sim_P = odeint(constant_expression, init_P, degradation_timecourse_minutes, args = p)[:,0]
    res = sim_P - degradation_timecourse_protein
    return res
...
# Import the degradation data and compute k_loss
def compute_k_loss(file_path):
    # Read in degradation data
    degradation_timecourse = pd.read_excel(file_path)
    degradation_timecourse_minutes = degradation_timecourse.iloc[:,0]
    degradation_timecourse_protein = degradation_timecourse.iloc[:,1]

    # ODE parameter estimate the model to get k_loss''' 
    n_steps = 100000 #fineness of timesteps
    sim1_t = np.linspace(min(degradation_timecourse_minutes), max(degradation_timecourse_minutes), n_steps) 
    init_P = [0] # Initial condition
    initial_guess = (100, 0.1)
    low_bounds = [0, 0]
    up_bounds = [1000, 1]
    fitted_params = least_squares(residuals_degradation_timecourse, initial_guess, bounds=(low_bounds, up_bounds), args=(init_P,degradation_timecourse_minutes,degradation_timecourse_protein)).x
    return fitted_params[1]


# Import the steady state data for constitutive promoters and compute k_express
def compute_k_express(file_path):
    constitutive_SS = pd.read_excel(file_path)
    vals = {}

    # Solve for k_express
    for x in range(0,4):
        SS_value = constitutive_SS.iloc[-1,1+x]
        vals[list(constitutive_SS)[1+x]] = calculate_kexpress(SS_value, k_loss)
    return vals


k_loss = compute_k_loss("degradation_timecourse.xlsx")
vals = {}
vals["k_loss"] = k_loss
vals["part"] = "cds"
vals["type"] = "reporter"
...
for prom, kexpress in compute_k_express("pLacIq_SS.xlsx").items():
    vals = {}
    vals["k_express"] = kexpress
    vals["part"] = "promoter"
